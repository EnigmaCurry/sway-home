...
piper() {
    ...
    local MODEL=${MODEL:-en_US-lessac-high}.onnx
...
EOF
    ...
    podman run --rm -i \
        ...
        -v ${MODEL_DIR}:/model:Z \
        ${IMAGE_NAME} \
        sh -c "piper -m /model/${MODEL} --length_scale ${SPEED} --output_raw $@ 2>${LOG}" | \
...
say() {
    ...
    if [ "$#" -gt 0 ]; then
        ...
        echo " " && cat
    fi | LOG=${LOG} MODEL_DIR=${MODEL_DIR} MODEL=${MODEL} piper;
}
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    alias piper="bash ${BASH_SOURCE[0]} piper"
fi
