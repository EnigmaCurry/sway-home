...
(defn config-nix [{:keys [user ssh-keys tz profiles]}]
  (str
   ...
   "  # --- System-wide /etc/hosts overrides ---\n"
   "  # /etc/hosts is a read-only symlink into the Nix store, so edit it\n"
   "  # declaratively here. Entries apply to every user and to root (handy\n"
   "  # for LAN hostnames, pinning a service to a specific IP, or blackholing\n"
   "  # a domain to 0.0.0.0). Rebuild with `admin switch` to apply.\n"
   "  # networking.hosts = {\n"
   "  #   \"192.168.1.10\" = [ \"nas\" \"nas.lan\" ];\n"
   "  #   \"10.0.0.5\"     = [ \"gitlab.example.com\" ];\n"
   "  #   \"0.0.0.0\"      = [ \"ads.example.com\" ];\n"
   "  # };\n"
   "  # # Or as a free-form /etc/hosts fragment:\n"
   ...
   "  # --- Optional: Bluetooth --------------------------------------------\n"
   "  # The sway profile is already wired to auto-start blueman-applet (tray\n"
   "  # icon) and has waybar/floating-window rules for blueman-manager; just\n"
   "  # enable the two services below to turn on the BlueZ stack and install\n"
   "  # blueman.\n"
   "  # hardware.bluetooth = {\n"
   "  #   enable = true;\n"
   "  #   powerOnBoot = true;\n"
   "  #   settings.General.Experimental = true;  # BLE battery reporting\n"
   "  # };\n"
