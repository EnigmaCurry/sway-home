...
(defn config-nix [{:keys [user ssh-keys tz]}]
  (str
   ...
   "  # The `vm` command (nixos-vm-template) drives libvirt. Uncomment to run\n"
   "  # the daemon, add yourself to the libvirtd group, then re-login so\n"
   "  # `vm list` can reach /var/run/libvirt.\n"
