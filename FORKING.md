...
The root `flake.nix` also references the emacs repo. Use the same flake
URL scheme as in step 6:
...
## Step 8: Point the installer at your fork

The installer tools reference the sway-home repo by flake ref. Update
both defaults to your fork:

- `default-sway-home-ref` in `bin/setup-host.bb` — baked into each
  generated host repo's `flake.nix` (its `inputs.sway-home.url`).
- `installer-flake-ref` in `bin/nix_build_iso.bb` — the tools baked
  into the ISO.

```bash
sed -i "s|EnigmaCurry/sway-home|${FORGE_USER}/${CONFIG_REPO}|g" \
  bin/setup-host.bb bin/nix_build_iso.bb
```

## Step 9: NixOS host configuration

This repo keeps **no** per-host config. Each machine gets its own small
flake repo (at `~/nixos`) generated by `setup host` during install,
which depends on your fork and calls `lib.mkHost`. Per-host values are
prompted at install time:

- `userName`, `hostName`, `timeZone` — asked by `setup host`.

Baked-in defaults you may want to change in `bin/setup-host.bb`:

- the keyboard layout defaults in the generated `flake.nix`
  (`layout = "us"`, `options = "ctrl:nocaps"`, ...),
- the default username/hostname/timezone offered at the prompts.
...
| `flake.nix` | Flake input for emacs |
| `bin/setup-host.bb` | sway-home ref; default username/hostname/timezone/keyboard |
| `bin/nix_build_iso.bb` | `installer-flake-ref` (sway-home ref baked into the ISO) |
